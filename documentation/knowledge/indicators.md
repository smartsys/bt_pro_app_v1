...
- **Level-Namen** = `<spec-id>_<param>` (seit der Umstellung), also `fast_sma_length`, `fast_sma_multiplier`, `supertrend_period`, `supertrend_multiplier`. `build_indicators` benennt jede Instanz direkt beim Bauen von VBTs Factory-Default (`<vbt-Klassenname kleingeschrieben>_<param>`, z.B. `dwsfastsma_length`) auf den **Spec-Key** um (`_rename_indicator_instance`). Grund: der Spec-Key ist pro Iteration eindeutig und über die ganze Kette hinweg konsistent — anders als der Klassenname, der bei zwei Instanzen derselben Klasse kollidiert und bei einem zugleich getragenen + direkt referenzierten Indikator zwei verschiedene Namen (Factory- vs. Spec-ID) für **dieselbe** Achse ergab (7x-Blowup, s. §6.6).
...
- **Chaining**: Ist ein Input eine Referenz `indicator:<id>:<out>` (z.B. ein nachgelagerter Indikator `B` mit `source = indicator:fast_sma:result`), erbt `B` die **Param-Level des Vorgängers** — und zwar unter dessen **Spec-ID-Namen**, weil der Vorgänger schon vor dem Konsum umbenannt wurde. Beispiel: die Columns von `B` tragen neben den eigenen Param-Leveln zusätzlich `fast_sma_length, fast_sma_multiplier` und `symbol`. Der ID-Name propagiert so über beliebig viele Kettenstufen (A→B→C). Die topologische Sortierung (`_topological_order`, Kahn) stellt sicher, dass Abhängigkeiten zuerst gebaut werden.

Damit hat jeder Indikator-Block **disjunkte** Param-Level (außer dem gemeinsamen `symbol`): die Kette aus `fast_sma`+`B` und `supertrend` teilen kein Param-Level. Wird ein verketteter Indikator zugleich **direkt** referenziert (z.B. `fast_sma` speist `vwma` UND eine Regel vergleicht direkt gegen `fast_sma`), tragen die getragene und die direkt referenzierte Achse **denselben** Namen (`fast_sma_length`) und werden von `_combine_broadcast` **gefaltet** statt gekreuzt — die geteilte Achse erscheint genau einmal im Portfolio.

> **Sauberer Schnitt — persistierte Param-Namen getragener Indikatoren.** Vor dem Fix trug ein **getragener** Indikator seine Achse unter dem Factory-Namen (`dwsfastsma_length`), ein direkt referenzierter unter dem Spec-ID-Namen (`fast_sma_length`). Ab dem Fix gilt **überall** der Spec-ID-Name. Diese Namen fließen in `actual_params_json`, `params_hash` und `backtest_result_params.param_name`. Folge: Results von **vor** dem Fix sind mit Results von **nach** dem Fix nicht mehr **namensgleich** — Cross-Run-Combo-Tracking (`lookup_results_across_runs`) und die `?<param>=value`-Filter finden alte Kombis eines getragenen Indikators nicht mehr wieder. Das ist ein **bewusster No-Match**, kein Bug: die Lookups wurden **nicht** dual-präfix gemacht, alte Results **nicht** zurückgeschrieben. Für vergleichbare Läufe die betroffenen Iterationen (z.B. iteration 7 und 2 der „Einfacher Pullback"-Familie) **frisch rechnen** — dann gilt innerhalb des neuen Namensschemas wieder durchgängige Vergleichbarkeit.
...
**Chunkweise Persistenz.** Ein gechunkter Multiparameterlauf schreibt seine Results nicht mehr erst am Ende, sondern nach jedem Chunk: der Spec-Runner reicht jeden fertigen Chunk an die vom Worker injizierte Senke, die ihn über `save_result_chunk` in einer eigenen Transaktion speichert — zusammen mit `backtest_runs.completed_chunks`, dem Fortsetzungspunkt. Ein hart beendeter Lauf behält damit die abgeschlossenen Chunks und wird über `POST /api/backtest/runs/{id}/resume` ab dem ersten fehlenden Chunk fortgesetzt. Den Lauf abschließend (Status, `n_combinations`, `ann_factor`, DSR-Nachlauf) macht `finalize_backtest_run` — einmal, über den ganzen Lauf, gechunkt wie ungechunkt derselbe Weg.

**Kennzahlen: keine Asymmetrie mehr.** Sie entstehen für jeden Lauf in genau einer Funktion — `_extract_metrics(portfolios, columns, backtest_config)`, vektorisiert über alle Spalten. Ein Result trägt denselben Satz Kennzahlen, unabhängig davon, ob der Lauf eine oder tausend Kombinationen hatte; die frühere Verzweigung `if n_combinations == 1` und die Spalte `metrics_level` sind entfernt.
...
- **Liste == Chart**: `_extract_metrics(portfolios)[combo] == _extract_metrics(einzeln gerechnetes Portfolio)[0]` — seit der Umstellung nicht mehr nur für `end_value`, sondern für **jede** Kennzahl, und nicht mehr als Vergleich zweier Implementierungen, sondern weil es nur noch einen Weg gibt.
...
- **Einmal-Migration (Skript entfernt)** — schrieb schreibt Kurz-IDs auf prefixte Katalog-IDs (`dwsFastSMA`→`custom:dwsFastSMA`, `supertrend`→`vbt:SUPERTREND`). Eindeutig (Custom-Modul oder genau ein VBT-Suffix-Match) → migriert; mehrdeutig/unauflösbar → unverändert + gemeldet.
- **Einmal-Migration (Skript entfernt)** — `enabled:true` → Feld entfernt. `enabled:false`: in editierbaren Configs/Setups/Iterationen (`IndicatorConfig`, `ChartPlaygroundSetup`, `StrategyIteration`) → ganzer Eintrag gelöscht; in historischen Lauf-Snapshots (`BacktestRun`, `TestSetRun`) → nur Feld entfernt, Eintrag bleibt.
